  ...
  methods: {
    modify(){
      ...
      api.getUserInfo(userId).then(result =>{
        ...
        // this.$set(this.form, 'username', result.user.username)
        // this.$set(this.form, 'password', result.user.password)
        // this.$set(this.form,'mobile', result.user.mobile)
        // this.$set(this.form,'email', result.user.email)
        // this.$set(this.form,'image', result.user.salt)
      }).catch(e => {
    ...
    secondConfirm(formName){
      this.$refs[formName].validate((valid) => {
        if (valid) {
          api.verifyPassword(this.secondForm).then(() => {
            let form1 = JSON.parse(JSON.stringify(this.form));
            form1.password = ''
            let form = this.oldPassword == this.form.password ? form1 : this.form
